...
def main():
    ...
    if not bios_str:
        ...
        exit(1)
    if not findall(r'P[0-9]{2}[A-z]{3}', bios_str):
        print 'Sorry, I only understand BIOS versions in the format P11AAA. Yours is %s' % bios_str
        exit(1)
    bios_version = int(bios_str[1:3])
    bios_model = bios_str[3:]
